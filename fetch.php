...
if ($_POST['query'] != '') {
  switch ($filter) {
    case 'email':
      $query .= 'WHERE cust_email LIKE "%' . str_replace(' ', '%', $_POST['query']) . '%" ';
      break;
    case 'name':
    default:
...
if (isset($page_array)) {
  ...
  for ($count = 0; $count < count($page_array); $count++) {
    if ($page == $page_array[$count]) {
      ...
      <a class="page-link" data-page_number="' . $count . '">' . $page_array[$count] . ' <span class="sr-only">(current)</span></a>
    </li>
    ';
